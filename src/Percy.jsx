...
export default function Percy({ session }) {
  ...
  const S = {
    ...
    phone:       { height:"100dvh", display:"flex", flexDirection:"column", position:"relative" },
    ...
    bottomNav:   { position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:390, background:colors.surface, backdropFilter:"blur(20px)", borderTop:`1px solid ${colors.border}`, display:"flex", justifyContent:"space-around", padding:"12px 0 28px", zIndex:100 },
        ...
        {inAppAlerts.map(({id,voucher:v})=>(
          <div key={id} style={{borderRadius:16,padding:"14px 16px",background:"rgba(251,146,60,0.08)",border:"1px solid rgba(251,146,60,0.25)",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
              <button onClick={()=>sendAllNotifications(v)} style={{fontSize:11,fontWeight:600,padding:"5px 12px",borderRadius:8,background:"rgba(251,146,60,0.2)",border:"1px solid rgba(251,146,60,0.4)",color:colors.warning,cursor:"pointer",whiteSpace:"nowrap"}}>{notifSent[v.id]?"✓ נשלח":"הודע לי"}</button>
        ...
        {!editingId&&(
          <>
            ...
            <div style={{padding:"16px 24px 10px",display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{fontSize:11,color:colors.textMuted,fontWeight:600}}>{scanState==="preview"?"בדוק וערוך":"או מלא ידנית"}</span>
              <div style={{flex:1,height:1,background:"rgba(255,255,255,0.07)"}}/>
            </div>
          </>
        )}
